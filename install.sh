...
#
# Usage: curl -fsSL https://raw.githubusercontent.com/itum/VortexL2/main/install.sh | sudo bash
#
...
REPO_URL="https://github.com/itum/VortexL2.git"
REPO_BRANCH="main"
...
if command -v git &> /dev/null; then
    git clone --depth 1 --branch "$REPO_BRANCH" "$REPO_URL" "$INSTALL_DIR" 2>/dev/null || {
        ...
        curl -fsSL "https://github.com/itum/VortexL2/archive/refs/heads/${REPO_BRANCH}.tar.gz" | \
...
# Install Python cryptography for secure forward (optional)
...
[ -f "$INSTALL_DIR/systemd/vortexl2-secure-fwd@.service" ] && \
    cp "$INSTALL_DIR/systemd/vortexl2-secure-fwd@.service" "$SYSTEMD_DIR/" || true
